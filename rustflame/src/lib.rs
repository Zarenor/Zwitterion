...
pub fn run(config: Config) -> Result<(), Box<dyn Error>> {
    ...
    let unprocessed;
    let _count = f.read_to_end(&mut bytes)?;
    ...
    match res {
        ...
        IResult::Err(Err::Error(err)) | IResult::Err(Err::Failure(err)) => {
            println!("Error reading \"flames\" tag: {:?}", err);
        }
        IResult::Err(Err::Incomplete(_needed)) => println!("Not enough characters were given..."),
    }
    Ok(())
}
named!(
